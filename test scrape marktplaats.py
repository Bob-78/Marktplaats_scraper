...
# create the lists that will form the columns of our table, so we can append to them
title = list()
km = list()
year = list()
price = list()
...
# we have listpages and carpages
...
def get_car_details(url):
    ...
    #print("attr.get(bouwjaar) is {}".format(attr.get("bouwjaar")))
    #print("attr.items is {}".format(attr.items()))
    print("attr.keys is {}".format(attr.keys()))
    #print("attr.values is {}".format(attr.values()))
    
    #values = [value for key, value in attr.items()] #values
    #pairs1 = [value for value in attr.items()] #pairs
    #pairs2 = [attr.items()]
    #print("value for key, value is {}".format(values))
    #print("value for value in items is {}".format(pairs1))
    #print("alleen attr.items is {}".format(pairs2))

    
    #with open("carresults.csv", "wb") as csv_file:
...
all_urls = get_all_urls(base_url, 2)

df = pd.DataFrame()

for url in all_urls:
    
    details = get_car_details(url) # this returns a dictionary all_urls[22] for single entry
